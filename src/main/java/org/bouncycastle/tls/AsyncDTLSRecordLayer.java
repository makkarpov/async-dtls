 ...
 */
class AsyncDTLSRecordLayer implements TlsCloseable {
    private static final int RECORD_HEADER_LENGTH = 13;
    public static final int MAX_FRAGMENT_LENGTH = 1 << 14;
    private static final int TCP_MSL = 1000 * 60 * 2;
    private static final int RETRANSMIT_TIMEOUT = TCP_MSL * 2;
    ...
    private ByteQueue recordQueue;

    private DTLSHandshakeRetransmit retransmit = null;
    private DTLSEpoch retransmitEpoch = null;
    private TlsTimer.TimerTask retransmitTimeout = null;
    private Runnable retransmitRunnable;
    ...
    private TlsTimer.TimerTask heartbeatTimeout = null;         // Idle timeout (if none in-flight), else expiry timeout for response
    private Runnable heartbeatRunnable;

    private int heartbeatResendMillis = -1;                     // Delay before retransmit of current in-flight heartbeat request
    private TlsTimer.TimerTask heartbeatResendTimeout = null;   // Timeout for next retransmit of the in-flight heartbeat request
    private Runnable heartbeatResendRunnable;
    ...
    public AsyncDTLSRecordLayer(TlsContext context, TlsTimer timer, TlsPeer peer, AsyncDTLSTransport transport) {
        ...
        heartbeatRunnable = new Runnable() {
            ...
            public void run() {
                try {
                    heartbeatTimoutExpired();
                } catch (IOException e) {
                    transport.exceptionCaught(e);
                }
            }
        };

        heartbeatResendRunnable = new Runnable() {
            @Override
            public void run() {
                try {
                    heartbeatResendTimeoutExpired();
                } catch (IOException e) {
                    transport.exceptionCaught(e);
                }
            }
        ...
        recordBuffer = new byte[MAX_FRAGMENT_LENGTH];
        recordQueue = new ByteQueue();
    }

    AsyncDTLSTransport getTransport() {
        return transport;
    }
    ...
    private void heartbeatTimoutExpired() throws IOException {
        ...
        this.heartbeatTimeout = schedule(heartbeatTimeout, heartbeatRunnable, heartbeat.getTimeoutMillis());

        this.heartbeatResendMillis = DTLSReliableHandshake.INITIAL_RESEND_MILLIS;
        this.heartbeatResendTimeout = schedule(heartbeatResendTimeout, heartbeatResendRunnable, heartbeatResendMillis);

        sendHeartbeatMessage(heartbeatInFlight);
    }

    private void heartbeatResendTimeoutExpired() throws IOException {
    ...
    private void resetHeartbeat()
    {
        ...
        this.heartbeatTimeout = schedule(heartbeatTimeout, heartbeatRunnable, heartbeat.getIdleMillis());
    }
    ...
    public void handshakeSuccessful(DTLSHandshakeRetransmit retransmit) {
        ...
        if (null != retransmit)
        {
            ...
            this.retransmitTimeout = schedule(this.retransmitTimeout, retransmitRunnable, RETRANSMIT_TIMEOUT);
        }
    ...
    // Async API: main method that should be invoked to push data
    public void pushReceivedData(byte[] data, int off, int len) throws IOException {
        // @makkarpov: a single malformed datagram can screw up whole queue if we retain it between calls
        // Additionally, DTLS RFC states that records cannot span multiple datagrams
        // So it is safe to clear queue at beginning of processing

        // However, this implies that users of this class must read everything (until -1 is returned) before next call

        recordQueue.removeData(recordQueue.available());
        recordQueue.shrink();
        recordQueue.addData(data, off, len);

        this.inConnection = true;
    }

    // Asynchronous version returns -1 when end of buffered data is reached
    // No 'len' parameter: original (sync) version didn't cared about it anyway
    private int receiveRecord(byte[] buf, int off) {
        ...
        int length = TlsUtils.readUint16(lengthBytes, 0);
    ...
    public int receive(byte[] buf, int off) throws IOException {
        while (true) {
            int received = receiveRecord(recordBuffer, 0);
            if (received < 0) {
                return -1;
            }

            int processed = processRecord(received, recordBuffer, buf, off);
            if (processed >= 0) {
                return processed;
            }
        }
            ...
            throws IOException
    {
        ...
        if (!inHandshake && null != retransmit)
        {
            ...
            if (this.retransmitTimeout != null) {
                this.retransmitTimeout.cancel();
            }
    ...
    private TlsTimer.TimerTask schedule(TlsTimer.TimerTask existing, Runnable target, int millis) {
        if (existing != null) {
            existing.cancel();
        }

        return timer.schedule(target, millis);
    }
